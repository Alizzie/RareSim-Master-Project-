...
from scripts.evaluation.benchmark_visualization.load_results import (
    load_all_results,
    load_case_agreement,
)
...
def q1_best_method(metrics: pd.DataFrame, output_dir: Path) -> None:
    ...
    pivot = metrics.pivot_table(
        index="method_label", columns="dataset", values="R@10", aggfunc="max"
    ...
    for i in range(n_rows):
        for j in range(len(order_cols)):
            ...
            ax.text(
                j, i, f"{val:.2f}", ha="center", va="center", fontsize=9, color=shade
            )

    ax.set_title("Q1 · Which method performs best? (Recall@10)")
    ax.grid(False)
    ax.set_xticks(np.arange(-0.5, len(order_cols), 1), minor=True)
...
def q2_recall_curves(metrics: pd.DataFrame, output_dir: Path, top_n: int) -> None:
    ...
    for dataset in datasets_in_order(metrics):
        ...
        for _, row in subset.iterrows():
            ...
            color = SYSTEM_TYPE_COLORS.get(
                type_map.get(row["method_label"], ""), "#888888"
            )
...
def q4_dataset_difficulty(
    metrics: pd.DataFrame, case_agreement: pd.DataFrame, output_dir: Path
) -> None:
    ...
    if not case_agreement.empty:
        hard = (
            case_agreement.groupby("dataset")["hard_case"]
            .mean()
            .reindex(order)
            .reset_index(name="hard_case_rate")
        )
    ...
    for xi, (b, m) in enumerate(
        zip(summary["best_recall10"], summary["mean_recall10"])
    ):
...
def q5_validation_vs_raresim(metrics: pd.DataFrame, output_dir: Path) -> None:
    ...
    pivot = best_by_system.pivot(
        index="dataset", columns="system_type", values="best_recall10"
    ).reindex(order)
    type_cols = [
        c
        for c in ["RareSim method", "Ensemble", "Validation tool"]
    ...
    for i, col in enumerate(type_cols):
        ax.bar(
            x + (i - (len(type_cols) - 1) / 2) * width,
            pivot[col],
            width,
            color=SYSTEM_TYPE_COLORS.get(col, "#888"),
            label=col,
        )
    ax.set_xticks(x)
...
def q6_ensemble_gain(metrics: pd.DataFrame, output_dir: Path) -> None:
    ...
    ax.bar(
        x - 0.2,
        df["best_single_recall10"],
        0.4,
        color=SYSTEM_TYPE_COLORS["RareSim method"],
        label="Best single method",
    )
    ax.bar(
        x + 0.2,
        df["best_ensemble_recall10"],
        0.4,
        color=SYSTEM_TYPE_COLORS["Ensemble"],
        label="Best RRF ensemble",
    )
    ...
    ax.set_ylabel("Recall@10")
    ax.set_ylim(
        0, ymax_for(df[["best_single_recall10", "best_ensemble_recall10"]].to_numpy())
    )
    ax.legend(loc="upper right")
...
def q7_family_overview(metrics: pd.DataFrame, output_dir: Path) -> None:
    ...
    family = metrics.groupby("method_family")["R@10"].mean().sort_values(ascending=True)
    ...
    colors = [
        (
            "#d1603d"
            if label == "Validation tool"
            else "#6a4c93" if label == "Ensemble" else "#2a7f9e"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("outputs/evaluation_visual_questions")
    )
    parser.add_argument(
        "--top-n", type=int, default=7, help="Methods shown in Q2 recall-curve plots."
    )
    ...
    metrics = load_all_results(args.raresim, args.validation, allowed_datasets=DATASETS)
    case_agreement = load_case_agreement(args.raresim, allowed_datasets=DATASETS)
