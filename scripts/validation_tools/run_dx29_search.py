...
def parse_args():
    p = argparse.ArgumentParser(
        description="Run Dx29 on benchmark datasets.",
...
def predict_case(
    hpo_list: list, host: str, lang: str, count: int
) -> tuple[bool, list | None]:
    """Predict diseases for a case with given HPO terms using Dx29 API."""
    url = f"{host}/api/v1/Search"
    params = {"skip": 0, "count": count, "lang": lang, "source": "all"}

    response = requests.post(url, params=params, json=hpo_list, timeout=30)

    if response.status_code == 200:
        data = response.json()
        return True, data.get("diseases", [])
    else:
        return False, None
...
def run_dataset(name: str, cases: list, args) -> list[dict]:
    ...
    for case_id, hpo_ids, ground_truth in entries:
        ...
        status, results = predict_case(hpo_ids, args.host, args.lang, args.topk)
...
def main():
    args = parse_args()
    workdir = VAL_OUTPUTS_DIR / "dx29_benchmarks"
